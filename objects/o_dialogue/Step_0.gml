...
if instance_exists(o_ada){
	...
	if keyboard_check_pressed(vk_space) && (alpa = 1 || alpa = 0){ //dialog kek gini gak efektic bet, harusnya dari awal gw bikin aja pagenya disini. too bad
		if (charcount < string_length(text)) && o_ada.page < 31  && skipable{
			charcount = string_length(text)	
		}else if(charcount = string_length(text)){
			if o_ada.page = 43 || (o_stats.usedcheat && o_ada.page > 37){
				bisa = false
				alarm[1] = 60
			}
			if o_ada.page = 30{
				fading = true
			}else if bisa && o_ada.page != 43 && !(o_stats.usedcheat && o_ada.page > 37){
				charcount = 0
			}
		}
	}
...
if keyboard_check_pressed(vk_space) && (alpa = 1 || alpa = 0){
	if (charcount < string_length(text)) && skipable{
		charcount = string_length(text)	
	}else if(charcount = string_length(text)) && nextpage{
		charcount = 0
	}
}
if fading{
